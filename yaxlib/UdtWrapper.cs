...
namespace YAXLib
{
    ...
    internal class UdtWrapper
    {
        ...
        private List<MemberWrapper> m_members;

        private List<MethodInfo> m_onDeserializedCallbacks = new List<MethodInfo>();
        ...
        public UdtWrapper(Type udtType, YAXSerializer callerSerializer)
        {
            ...
            this.Comment = null;
            this.FieldsToSerialize = YAXSerializationFields.AttributedFieldsOnly;
            ...
            foreach (var method in m_udtType.GetAllMethods())
        ...
        public IEnumerable<MemberWrapper> GetMembers(YAXSerializer ser)
        {
            if (m_members == null)
            {
                m_members = new List<MemberWrapper>();

                var fields = UnderlyingType.GetAllFields(false).Cast<MemberInfo>();
                var props = UnderlyingType.GetAllProperties().Cast<MemberInfo>();

                foreach (var member in fields.Concat(props))
                {
                    ...
                    {
                        MemberWrapper memInfo = new MemberWrapper(member, ser, this);

                        if (memInfo.IsAllowedToBeSerialized(FieldsToSerialize))
                            m_members.Add(memInfo);
                    }
                }
            }
        ...
        public IEnumerable<MethodInfo> OnDeserializedCallbacks
        {
            get
            {
                return m_onDeserializedCallbacks;
            }
        }
        ...
        /// </value>
        public bool IsAttributedAsNotCollection { get; set; }
        ...
        public bool IsOuterCollection
        {
            get
            {
                return GetMembers(null).Any(m => m.IsInnerCollection);
            }
        }
        ...
        private void ProcessMethodAttributes(MethodInfo method)
        {
            foreach (var attrib in method.GetCustomAttributes(true))
            {
                if (attrib is YAXOnDeserializedAttribute)
                {
                    if (method.GetParameters().Length > 1)
                        throw new InvalidOperationException();
                    m_onDeserializedCallbacks.Add(method);
                }
            }
        }
        ...
        private void ProcessYAXAttribute(object attr)
        {
            ...
            {
                YAXSerializeAttribute theAttr = attr as YAXSerializeAttribute;
