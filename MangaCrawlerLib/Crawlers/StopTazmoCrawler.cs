    ...
    internal class StopTazmoCrawler : Crawler
    {
            ...
            IEnumerable<SerieInfo>> a_progress_callback)
        {
            ...
            var series = doc.DocumentNode.SelectNodes(
                "/html/body/div[2]/div/div/div/ul/li/table[2]/tr/td[1]/a");

            var result = from serie in series
                         where serie.InnerText.Trim() != "[LATEST_DOWNLOADS]"
                         where serie.InnerText.Trim() != "[VOLUMES]"
                         select new SerieInfo(a_info, serie.GetAttributeValue("href", ""), 
                             serie.InnerText);
            ...
            IEnumerable<ChapterInfo>> a_progress_callback)
        {
            ...
            var chapters = doc.DocumentNode.SelectNodes(
                "/html/body/div[2]/div/div/div/ul[2]/li/table/tr");
            ...
            CancellationToken a_token)
        {
            ...
            var pages = doc.DocumentNode.SelectNodes(
                "//select[@class='selectpage']/option");
            var post_url = doc.DocumentNode.SelectSingleNode(
                "/html/body/div[2]/div/div/table/tr/td/form").GetAttributeValue("action", "");
        ...
        internal override string GetImageURL(PageInfo a_info, CancellationToken a_token)
        {
            ...
            HtmlDocument doc = ConnectionsLimiter.Submit(a_info, a_token, ar[3],
                new Dictionary<string, string>() { 
                    { "manga_hid", ar[0] }, { "chapter_hid", ar[1] }, { "image_hid", ar[2] }, 
                    { "series", ar[0] }, { "chapter", ar[1] }, { "pagesel1", ar[2] }});

            var image = doc.DocumentNode.SelectSingleNode(
                "/html/body/div[2]/div/div/table/tr/td").SelectSingleNode("table/tr[2]/td/a/img");
