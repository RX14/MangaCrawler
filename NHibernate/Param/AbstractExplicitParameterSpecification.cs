...
namespace NHibernate.Param
{
	public abstract class AbstractExplicitParameterSpecification : IExplicitParameterSpecification
	{
		...
		protected int GetParemeterSpan(IMapping sessionFactory)
		{
			...
			{
				// xTODO: we have to find a way to set all expected types during the query parsing
			...
			// xTODO: (see above) when the ExpectedType is null we will set the BackTrackId just for the first position (not a big problem because IType does not support something different... so far)
			return 1;
		}
	}
}