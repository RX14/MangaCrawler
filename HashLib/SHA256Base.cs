...
namespace HashLib
{
    internal abstract class SHA256Base : HashCryptoNotBuildIn
    {
        ...
        {
            uint[] block = new uint[64];
            Converters.ConvertBytesToUIntsSwapOrder(a_data, a_index, BlockSize, block);
            ...
            for (int r = 16; r < 64; r++)
            {
                uint T = block[r - 2];
                uint T2 = block[r - 15];
                block[r] = (((T >> 17) | (T << 15)) ^ ((T >> 19) | (T << 13)) ^ (T >> 10)) + block[r - 7] +
                    (((T2 >> 7) | (T2 << 25)) ^ ((T2 >> 18) | (T2 << 14)) ^ (T2 >> 3)) + block[r - 16];
            }

            for (int r = 0; r < 64; r++)
            {
                uint T = s_K[r] + block[r] + H + (((E >> 6) | (E << 26)) ^ ((E >> 11) | (E << 21)) ^ ((E >> 25) |
                         (E << 7))) + ((E & F) ^ (~E & G));
